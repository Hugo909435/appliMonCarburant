...
import '../../providers/stations_provider.dart'
    show priceHistoryServiceProvider;
          ...
          Card(
            child: Padding(
              ...
              child: historyAsync.when(
                data: (points) {
                  final withFuel = points
                      .where((p) => p.prices.containsKey(fuel.code))
                      .toList();
                        ...
                        lineBarsData: [
                          LineChartBarData(
                            ...
                            belowBarData: BarAreaData(
                              show: true,
                              color: fuel.color.withValues(alpha: 0.12),
                            ),
                            spots: [
                              for (var i = 0; i < withFuel.length; i++)
                                FlSpot(
                                  i.toDouble(),
                                  withFuel[i].prices[fuel.code]!,
                                ),
                            ],
                          ),
                        ],
                ...
                error: (err, _) =>
                    const Text("Impossible de charger l'historique des prix."),
              ),
            ),
          ),
        ],
